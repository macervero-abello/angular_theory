# Capítol 3. Components de navegació amb Ionic
...
A més a més, tot i no estar expressament pensades per això, les aplicacions fetes amb Ionic també es poden compilar per a ser utilitzades en navegadors, per tant, el *framework* també ofereix la navegació mitjançant una barra lateral (*sidebar*)

A continuació s'expliquen les tres metodologies per a mòbils (botons, menú i pestanyes).
...
{% code title="Codi app.component.html" overflow="wrap" lineNumbers="true" %}
  ```html
      <ion-app>
...
Tot i que Ionic v8 és completament compatible amb Angular v20, manté la nomenclatura antiga dels *components*, per tant, el *component* principal s'anomena `AppComponent` i els seus fitxers mantenen el sufix: `app.component.html`, `app.component.ts` i `app.component.scss`.
{% endhint %}

Tornant a l'exemple del codi, l'etiqueta `<ion-app>` s'encarrega d'iniciarlitzar la part gràfica de l'aplicació i l'etiqueta `<ion-router-outlet>` actua de contenidor de rutes.
...
ionic generage page view/about
```
Podem navegar d'una pàgina a l'altra de la manera que mostra el codi següent:

{% tabs %}
{% tab title="Codi home.page.html" overflow="wrap" lineNumbers="true" %}
  ```html
    ...
    <ion-content [fullscreen]="true">
      <ion-button [routerLink]="['/about']">Sobre l'aplicació</ion-button>
    </ion-content>
  ```
{% endtab %}

{% tab title="Codi home.page.ts" overflow="wrap" lineNumbers="true" %}
  ```typescript
    ...
    @Component({
      ...
      standalone: true,
      imports: [IonContent, IonHeader, IonTitle, IonToolbar, IonButton, RouterModule]
    })
...
{% tab title="Codi about.page.html" overflow="wrap" lineNumbers="true" %}
  ```html
    <ion-header [translucent]="true">
      <ion-toolbar>
        <ion-title>Sobre l'aplicació</ion-title>
      </ion-toolbar>
    </ion-header>

    <ion-content [fullscreen]="true">
      <ion-button (click)="onGoToHome()">Inici</ion-button>
    </ion-content>
  ```
{% endtab %}

{% tab title="Codi about.page.ts" overflow="wrap" lineNumbers="true" %}
  ```typescript
    import { Component, OnInit } from '@angular/core';
    import { IonContent, IonHeader, IonTitle, IonToolbar, IonButton } from '@ionic/angular/standalone';
...
Suposem que tenim tres pàgines, la `HomePage`, la `ListPage` i l'`AboutPage`, amb les rutes `/home`, `/list` i `/about` definides respectivament i de manera automàtica per cadascuna d'elles en el moment d'executar les comandes:
```bash
ionic generate page view/home
          ...
          <ion-list>
            <ion-menu-toggle>
              <ion-item routerLink="['/home']" [routerLinkActive]="['activatedLink']">
                <ion-label><ion-icon name="home-outline" slot="start"></ion-icon> Inici</ion-label>
              </ion-item>
            </ion-menu-toggle>
            <ion-menu-toggle>
            ...
            <ion-menu-toggle>
              <ion-item [routerLink]="['/about']" [routerLinkActive]="['activatedLink']">
                <ion-label><ion-icon name="information-outline"></ion-icon> Sobre l'aplicació</ion-label>
              </ion-item>
            </ion-menu-toggle>
          </ion-list>
        </ion-content>
...
{% tab title="Codi app.component.scss" overflow="wrap" lineNumbers="true" %}
  ```scss
    .activatedLink {
...
Addicionalment, el codi `TS` del fitxer `app.component.ts` és el següent:

{% tab title="Codi app.component.ts" overflow="wrap" lineNumbers="true" %}
  ```typescript
    import { Component } from '@angular/core';
    import { RouterModule } from '@angular/router';
    import { IonApp, IonRouterOutlet,IonMenu, IonHeader, IonToolbar, IonTitle, IonContent, IonList, IonMenuToggle, IonItem, IonLabel, IonIcon } from '@ionic/angular/standalone';
    ...
    @Component({
      ...
      imports: [IonApp, IonRouterOutlet, IonMenu, IonHeader, IonToolbar, IonTitle, IonContent, IonList, IonMenuToggle, IonItem, IonLabel, IonIcon, RouterModule],
    })
    export class AppComponent {
      constructor() {
        addIcons({ homeOutline, informationOutline });
      }
    }
  ```
{% endtab %}

En aquest codi cal ressaltar la necessitat de fer un `import` de totes i cadascuna de les etiquetes d'Ionic, així com també del mètode `addIcons()`, necessari per carregar totes les icones de la llibreria IonIcons que es volen utilitzar.

Aquest exemple crea el menú que es mostra en la següent imatge:

![Visualització del menú creat en l'exemple d'aquest apartat](img/ch03/ionic_menu_example.png)
