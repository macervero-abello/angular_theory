# Capítol 6. Lector de codis de barres
...
Tal com s'ha dit en el [Capítol 4](chapter04.md), Capacitor és una eina de l'ecosistema Ionic que no només permet compilar codi Ionic i crear l'executable segons la plataforma per la qual s'estigui implementant, sinó que també ofereix un conjunt de [*pluguins*](https://capacitorjs.com/docs/plugins) que permeten l'accés directe al *hardware* del dispositiu. Aquests *pluguins* poden ser oficials (fets per l'equip desenvolupador de Capacitor) o de la comunitat (fets per desenvolupadors externs i validats per l'equip de l'ecosistema d'Ionic).

En concret, dins dels *pluguins* de comunitat en trobem dos que permeten utilitzar la càmara del mòbil per crear un lector de codi de barres, sigui del tipus que sigui (EAN13, QR, ISBN, etc.):
* [Capacitor Barcode Scanner](https://capacitorjs.com/docs/apis/barcode-scanner) (oficial) i
* [Capacitor ML Kit Barcode Scanning Plugin](https://capawesome.io/plugins/mlkit/barcode-scanning/) (comunitari).

{% hint style="warning" %}
Cal tenir en compte que el *plugin* oficial permet crear un escànner sobre qualsevol plataforma (Antroid, IOs i navegador web) però, la seva configuració visual i estètica és extremadament limitada.

En canvi, el *plugin* comunitari només funciona sobre les plataformes mòbils (Android i IOs) però, la seva configuració i personalització és molt més flexible i versàtil.
{% endhint %}

Els passos a seguir per aconseguir implementar el lector són els següents, sigui quin sigui el *plugin* utilitzat:
...
## Capacitor Barcode Scanner (oficial)

### Instal·lació del *plugin*
Per poder instal·lar aquest *pluguin* cal comprovar que el projecte Ionic té instal·lat Capacitor. Si aquest no és el cas, caldrà instal·lar-lo seguint els passos indicats al [primer apartat del Capítol 4](chapter04.md#installar-capacitor).

Fet això, ja es pot procedir a instal·lar el *pluguin* mitjançant i, tot seguint, sincronitzar el codi per tal que els projectes Android i IOs quedin actualitzats. Les comandes per fer-ho són les següents:
```bash
...
Per poder crear el codi de l'aplicació és important analitzar la [documentació oficial](https://capacitorjs.com/docs/apis/barcode-scanner) del *plugin* i, més específicament, la seva [API](https://capacitorjs.com/docs/apis/barcode-scanner#api), on es troben recollides les funcions que ofereix per poder transformar la càmera del dispositiu en un lector de codi de barres.

De fet, el *plugin* només ofereix un únic mètode:
1. [`scanBarcode()`](https://capacitorjs.com/docs/apis/barcode-scanner#scanbarcode): en cas de necessitar-ho, demana a l'usuari que activi els permisos de la càmera i, tot seguit, realitza la lectura del codi de barres tenint en compte un conjunt d'opcions de configuració ([`CapacitorBarcodeScannerOptions`](https://capacitorjs.com/docs/apis/barcode-scanner#capacitorbarcodescanneroptions)) que especifiquen, entre altres coses,
    * els formats de codi que es poden llegir;
    * quina càmera s'ha d'utilitzar en els dispositius mòbils (la del davant o la del darrere);
    * l'orientació de la càmera (horitzontal o vertical) i
    * la llibreria que cal utilitzar (ZXING o MLKIT).

Un cop estudiada la documentació ja es pot passar a implementar el `BarcodeScannerService` per tal que ofereixi les funcions bàsiques per inicialitzar el lector (la càmera del sistema) i fer la captura del codi de barres:

```bash
    ionic generate service service/barcode-scanner
```

Un cop fet el *service*, només faltarà implementar la part de la *view* en una de les pàgines de l'aplicació.

#### Implementació del *service*
Tal com s'ha vist en l'apartat anterior, per poder executar el lector s'ha d'utilitzar el mètode `scanBarcode()`. Per tant, el codi del *service* queda de la manera següent:
...
El mètode `scan()` del *service* `BarcodeScannerService` crida a la funció `scanBarcode()` del *pluguin*. En el cas de l'exemple, aquest mètode rep un conjunt d'opcions que permetran que l'escàner 
* funcioni amb la càmera posicionada en vertical;
* pugui llegir codis de barres en els formats QR i EAN-13 (el clàssic de barres verticals) i
* mostri un botó amb el títol `Escaneja!`, el qual llençarà l'event de lectura.

El retorn del mètode és un objecte de tipus `Promise<CapacitorBarcodeScannerScanResult>`, el qual conté el resultat de la lectura (`ScanResult`) en format `string` i el format de codi que s'ha llegit (`CapacitorBarcodeScannerTypeHint`). Aquest resultat es guarda dins d'un atribut `private` de tipus `WritableSignal` que té associat el seu propi `Signal` `public` per poder actuar com a *getter*.
...
Un cop fet això, només necessitem crear una pàgina que tingui un botó que activi la càmera per poder llegir el codi de barres desitjat i un element de text que mostri el contingut del codi llegit.
...
import { IonContent, IonFab, IonFabButton, IonIcon, IonLabel, IonTitle, IonToolbar, IonHeader } from '@ionic/angular/standalone';
...
export class BarcodeScannerPage {
  private _barcodeScanner: BarcodeScannerService = inject(BarcodeScannerService);
  public barcode: Signal<string> = computed(() => {return this._barcodeScanner.lastBarcode();});

  constructor() {}

  pulblic async onScan(): Promise<void> {
    await this._barcodeScanner.scan();
  }
}
```
{% endtab %}
...
Com es pot comprovar, el codi d'aquesta pàgina injecta el service que s'ha implementat prèviament (`BarcodeScannerService`) i només té un mètode: `onScan()`: activa la càmera i fa la lectora. La comunicació entre el *service* i la pàgina es fa a través dels `Signals` fet que permet que les dades s'actualitzin automàticament.
...
La primera instrucció crearà la carpeta `android`, preparada per a ser oberta des d'*AndroidStudio*, amb tots els fitxers necessaris per poder configurar i compilar l'aplicació per a dispositius Android. La segona actualitzarà els fitxers de la carpeta amb la última versió del nostre codi *Ionic/Angular*.

{% hint style="warning" %}
És possible que per poder fer la sincronització correctament, abans s'hagi d'executar un `build`
```bash
    ionic capacitor add android
...
La primera instrucció crearà la carpeta `ios`, preparada per a ser oberta des d'*XCode*, amb tots els fitxers necessaris per poder configurar i compilar l'aplicació per a dispositius IOS. La segona actualitzarà els fitxers de la carpeta amb la última versió del nostre codi *Ionic/Angular*.
...
Per configurar l'aplicació Android només cal assegurar que la versió de l'*Android SDK* sigui igual o superior a la 26. Per fer-ho, cal afegir al fitxer `variables.gradle`, que es troba dins de la carpeta `android/`, la línia següent:
```json
    ext {
        minSdkVersion = 26,
        ...
...
Si ja s'han fet tots els passos anteriors, l'aplicació ja pot ser compilada, executada i provada. Per fer-ho, es poden seguir els passos del [Capítol 4](./chapter04.md), i d'aquesta manera poder utilitzar la funcionalitat *Forward port* sobre el mòbil o, en canvi, crear i instal·lar l'APK directament, tal com s'explica al [Capítol 5](./chapter05.md).

<!--
## Capacitor ML Kit Barcode Scanning Plugin (comunitari)

### Instal·lació del *pluguin*
Tal com ja s'ha explicat en l'[apartat anterior](#capacitor-barcode-scanner-oficial), primer cal comprovar que el projecte Ionic té instal·lat Capacitor. Si aquest no és el cas, caldrà instal·lar-lo seguint els passos indicats al [primer apartat del Capítol 4](chapter04.md#installar-capacitor).

Fet això, ja es pot procedir a instal·lar el *pluguin* mitjançant les comandes següents (recordar que després de la instal·lació cal sincrinitzar els projectes):
```bash
